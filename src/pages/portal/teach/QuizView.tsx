  ...
  if (taking) {
    return (
      <div style={{ padding:24, maxWidth:800, margin:'0 auto', fontFamily:'sans-serif' }}>
        <button onClick={() => { setTaking(null); setLastScore(null); }} style={{ background:'none', border:'none', color:C.primary, cursor:'pointer', marginBottom:14, fontSize:13 }}>← Back to quizzes</button>
        <h2 style={{ margin:'0 0 16px', color:C.text }}>{taking.title}</h2>
        {lastScore ? (
          ...
          <>
            {attemptQs.map((q,qi) => (
              <div key={q.id} style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:6, padding:16, marginBottom:12 }}>
                <div style={{ fontSize:12, color:C.muted, marginBottom:6 }}>Q{qi+1} • {q.points} pts</div>
                <p style={{ margin:'0 0 12px', fontSize:14, color:C.text }}>{q.prompt}</p>
                {q.question_type === 'multiple_choice' && q.options.map((o,oi) => (
                  <label key={oi} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 0', cursor:'pointer', fontSize:13, color:C.text }}>
                    <input type="radio" name={`q-${q.id}`} checked={answers[q.id!]===oi} onChange={() => setAnswers(a => ({ ...a, [q.id!]: oi }))} style={{ accentColor:C.primary }}/>
                    {o.text}
                  </label>
                ))}
                {q.question_type === 'true_false' && [{v:0,l:'True'},{v:1,l:'False'}].map(o => (
                  <label key={o.v} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 0', cursor:'pointer', fontSize:13 }}>
            ...
            <button onClick={submitAttempt} style={{ padding:'10px 24px', border:'none', borderRadius:6, background:C.primary, color:'white', fontSize:14, fontWeight:600, cursor:'pointer' }}>Submit Quiz</button>
          </>
        )}
      </div>
    );
