...
const Dashboard: React.FC<Props> = ({ onEnterCourse }) => {
  ...
  const handleDuplicate = async (src: DBCourse) => {
    ...
    try {
      // Server-side deep clone (service role) so no content is silently skipped by RLS.
      const { data, error } = await supabase.functions.invoke('duplicate-course', {
        body: { sourceCourseId: src.id, title: newName.trim(), code: newCode.trim() },
      });
      if (error) throw error;
      if ((data as any)?.error) throw new Error((data as any).error);

      const stats = ((data as any)?.stats ?? {}) as Record<string, number>;
      await loadCourses();
      const summary = Object.entries(stats).map(([k, v]) => `${v} ${k.replace(/_/g, ' ')}`).join(', ');
      alert(`Created "${newName}" as a full sandbox copy.\n\nCopied: ${summary || 'nothing found in source course'}`);
    } catch (err: any) {
